"""CoarseNet FCN minutiae detector (PyTorch), the FCN stage of MinutiaeNet.

Port of CoarseNetmodel (deploy) from MinutiaeNet (github.com/luannd/MinutiaeNet, MIT),
verified to reproduce the TF1 deploy maps. Adds residual add() blocks and a deeper
backbone vs FingerNet; label2mnt negates the angle. Fully convolutional.
"""
...
class CoarseNet(nn.Module):
    def __init__(self, src: WSrc | None = None):
        ...
        def cbp(name, in_ch, out_ch, k, dil=1):
            ...
            self.cbp[name] = CBP(src, cn, "bn-" + name, "prelu-" + name, in_ch, out_ch, k, dil)

        def proj(name, in_ch, out_ch):
            self.proj[name] = src.conv(name, in_ch, out_ch, 1)
    ...
    def forward(self, img):
        ...
        fr, fi = self.gabor_real(img), self.gabor_imag(img)
        ...
        return {"seg_out": seg_out, "mnt_o_out": mnt_o, "mnt_w_out": mnt_w,
                "mnt_h_out": mnt_h, "mnt_s_out": mnt_s}


def label2mnt(mnt_s, mnt_w, mnt_h, mnt_o, thresh=0.5):
    """CoarseNet label2mnt: angle negated vs FingerNet. (N,4) x,y,angle,score."""
